...
//本js文件常用参数
// var zdo_themeurl=zfunc_themeurl();
...
zdo_modules_S.zShowBox = function() {
	...
	function zShowBox(domChunk) {
		...
		$(domChunk+' a.zshowbox').click(function(){
			...
			if (typeof document.body.style.maxHeight === "undefined") { //if IE 6
				...
				+ '<p id="zshowbox_loading" style="font-size:22px;background-color:#000;"></p>'
				+ '<div id="zsb" style="'+css_zsb+'">'
						+ '<img id="zsb_img" style="'+css_zsb_img+'" />'
						+ '<p id="zsb_prev" style="left:-30px;'+css_zsb_p_n+'">&laquo;</p>'
						+ '<p id="zsb_next" style="right:-30px;'+css_zsb_p_n+'">&raquo;</p>'
				...
				$zshowbox_loading.fadeTo(1,0.85).zdo_loading();
	...
	function zsh_img(img_id,zsb_img_url,pagesize,current,zcounter) { //图片放大load function
		...
		var ni = new Image();
		ni.onload = function(){
			var x = pagesize[0] - 100, y = pagesize[1] - 100, img_w=ni.width, img_h=ni.height;
			if (img_w > x) {
				img_h = img_h * (x / img_w);
				img_w = x;
				if (img_h > y) {
...
zdo_modules_S.AjaxComment = function() {
	var $commentform = $('#commentform'),
			comment_ajax_zww_php_file = zdo_ajax_url.home_url == 'http://z.turn/zww' ? 'comments-ajax-zww-local.php' : 'comments-ajax-zww.php';
			ajax_php_url=zdo_themeurl+'/'+comment_ajax_zww_php_file,
			...
			pic_sb = ajax_php_url.replace(comment_ajax_zww_php_file,'')+'img/spinner.gif',//提交ajax_c_loading图片位置
